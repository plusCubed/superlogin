...
module.exports = function(router, passport, user, config) {
    ...
    return user.createSession(req.user._id, provider, req)
      .then(function(mySession) {
        return BPromise.resolve({
          ...
          session: mySession,
          link: null,
            devTargetOrigin: config.getItem('devTargetOrigin'),
            prodTargetOrigin: config.getItem('prodTargetOrigin')
        });
      })
  ...
  function passportCallback(provider, options, operation) {
    return function(req, res, next) {
      ...
      theOptions.callbackURL = options.callbackURL || getLinkCallbackURLs(provider, req, operation, accessToken);
